...
public static String[] template(String args[]){
	...
	if(args[1].equals("Who")&&args[2].equals("is-a")&&args[3].equals("student")){
		ans[0]="is-a"; ans[1]="?x";ans[2]="student";
	}else if(args[1].equals("What")&&args[2].equals("does")&&args[3].equals("Shinkai")&&args[4].equals("like")){
		ans[0]="like"; ans[1]="Shinkai";ans[2]="?x";
	}
	...
	return(ans);
	   ...
	   public static void main(String args[]){
			...
			//example[q 1 friends Shinkai ?y]
				...
				//example[? Who is a student]
				for(int i=0;i<n/3;i++){
					sn.addLink(new Link(data[3*i],data[3*i+1],data[3*i+2],sn));
				}
				ArrayList<Link> query = new ArrayList<Link>();
				String ans[] = template(args);
				query.add(new Link(ans[0],ans[1],ans[2]));
				sn.query(query);
